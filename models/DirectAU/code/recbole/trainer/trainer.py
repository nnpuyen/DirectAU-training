...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        check_func = getattr(self.model, 'check', None)
        total_loss = None
        total_check_loss = None
        iter_data = (
            tqdm(
        ...
        for batch_idx, interaction in iter_data:
            ...
            if callable(check_func):
                check_losses = check_func(interaction)
                check_loss_tuple = tuple(per_loss.item() for per_loss in check_losses)
                total_check_loss = check_loss_tuple if total_check_loss is None else \
                    tuple(map(sum, zip(total_check_loss, check_loss_tuple)))
        ...
        if isinstance(total_loss, tuple):
            total_loss = tuple(per_loss / len(train_data) for per_loss in total_loss)
        else:
            total_loss /= len(train_data)
        if total_check_loss:
            total_check_loss = tuple(per_loss / len(train_data) for per_loss in total_check_loss)
            print(total_check_loss)
        return total_loss
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        if load_best_model:
            ...
            self.logger.info(message_output)
            save_func = getattr(self.model, 'save_params', None)
            if callable(save_func):
                save_func()  # !!!!! manually save params
...
class BUIRTrainer(Trainer):
    def __init__(self, config, model):
        super(BUIRTrainer, self).__init__(config, model)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in iter_data:
            ...
            if self.clip_grad_norm:
                clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
            self.optimizer.step()
            self.model.update_target()  # update target after each step
